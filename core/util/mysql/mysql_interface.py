...
class MysqlInterface:
    ...
    def __getMysqlConnection(self):
        return MySql(self.mysqlConf)


    def queryOne(self,sql):
        mysqlConnection = self.__getMysqlConnection()
        mysqlConnection.query(sql)
        return mysqlConnection.fetchOneRow();

    u'查询返回数据'
    def queryAll(self,sql):
        mysqlConnection = self.__getMysqlConnection()
        mysqlConnection.query(sql)
        return mysqlConnection.fetchAllRows();
    ...
    def dropTable(self,dbTbName):
        ...
        if (mysqlConnection.update(dropTableSql) == True):
            return True
        else:
            return False
    ...
    def tableInfo(self,dbName,tbName):
        ...
        mysqlConnection.query(sql)
        data = mysqlConnection.fetchOneRow()
        result = {
    ...
    def updataData(self,upSql):
        ...
        if (mysqlConnection.update(upSql) == True):
            return True
        else:
            return False
    ...
    def batchExecuteSql(self,sqlContent):
        ...
        status = mysqlConnection.executeSqls(formatSqlContentList)
